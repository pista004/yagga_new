...
class Admin_Model_OrderMapper {
    ...
    public function getDbTable() {
        ...
        if (!$this->_dbTable instanceof Admin_Model_Order) {

            $this->setDbTable("Admin_Model_DbTable_Order");
        }

        return $this->_dbTable;
    }

    public function getOrders($page = 0, $ipp = -1, $order = "") {
        ...
        $select = $this->getDbTable()
                ...
                ->joinLeft(array('os' => 'order_state'), 'o.order_order_state_id = os.order_state_id', array('os.*'))
        ...
        foreach ($result as $row) {
            ...
            $invoice->setOptions($row->toArray());
            $order->setOrder_invoice($invoice);

            $entries[] = $order->setOptions($row->toArray());
        }
