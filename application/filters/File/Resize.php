 ...
 * @package    Skoch_Filter
 */
class Filter_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'Filter_File_Resize_Adapter_Gd';
    ...
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'Filter_File_Resize_Adapter_') {
                    $name = 'Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
